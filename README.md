...
「連続行動の抑制」と「ランダム性」のバランス調整
擬似乱数を用いた確率判定を導入することで、AIに一定の予測不能性を持たせつつ、特定行動が連続しないように**LastActionを記録・監視**し、連続行動を制御するロジックを実装しました。これにより、自然かつ多様な行動パターンを実現しました。
...
【成果】 このロジックの実装により、プレイヤーは戦闘フェーズや距離に応じて戦略を切り替える必要が生まれ、AIの単調さを解消し、手応えのある戦闘体験を提供できました。


**Ⅱ. プレイヤーの位置と入力への動的な対応**
